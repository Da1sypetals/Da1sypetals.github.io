...
= 问题
对于*实对称*矩阵 $A in RR^(n times n)$ 和向量 $b in RR^n$，求解
$
  A x=b
$
或者，等价的，
...
+ 最速下降的新方向：$r_(i+1)$
  ...
  #image("./assets/grad.jpg", width: 60%)
...
- 新方向与前一步下降方向 $r_i$ #strike[垂直] 斜率之积为 $-a^(-2) b^2$ (@ejjl)
  - 这个方向由最速下降的方向进行一些小改动得到，我们可以在后面的算法部分(@algo)看到。把这个方向从和前一个搜索方向#strike[*垂直*]改动到*斜率之积为 $-a^(-2) b^2$* 就是CG和最速下降唯一不同的地方。
  #image("./assets/cggrad.jpg", width: 75%)
- 步长 $alpha$：由于是在一条直线上做优化，因此和最速下降的 $alpha$ 相同。
由于一次迭代只涉及到两个点、两个向量，只能构成一个平面，我们甚至不需要将二维向多维推广。
  #text(size:11pt)[- 若需推导，我们需要做的只是把点的 $n$ 维坐标映射到二维，然后对截面椭圆对应的二阶二次型进行SVD获得其长轴 $a$ 和短轴 $b$，进而根据其离心率以及上述斜率积的二级结论计算两个方向的关系。这里不展开。]

== 算法 <algo>
=== 初始化
算法输入：$A, b, x_0$
#image("./assets/algo-init.jpg", width: 35%)
=== 算法过程
#image("./assets/algo.jpg", width: 35%)
