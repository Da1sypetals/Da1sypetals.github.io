...
2. 启动一个 sstwriter，写入这个 memtable
  a. 一个 memtable 对一个 sst
3. 等到写入 sst 写完之后，才进行：
  a. 从 frozen memtables 、frozen memtable sizes 里面删除这个 memtable
  b. 从 wal 里面删除这个 memtable 对应的 wal
  c. update manifest
== Try Freeze
如果当前大小>freeze size 那么就 freeze；进一步如果所有 frozen memtable 大小之和> flush threshold，那么久 set flush signal。
== 写操作
1. 写 memtable
2. 写 WAL
3. try freeze
...
3. 如果 userkey 不匹配，说明不存在timestamp 小于当前 timestamp 的记录，返回（未找到）。
...
= Bloom filter 细节，_by Deepseek_
该Bloom filter算法的主要步骤如下：
1. 参数计算：
- 根据预期元素数量n和可接受误判率p，通过公式计算最优位数m和哈希函数数量k：
  - m = ⌈-n·ln(p)/(ln2)^2⌉
  - k = ⌈(m/n)·ln2⌉
- 当直接指定参数时，使用给定的位数和哈希函数数量
2. 哈希生成：
- 使用64位指纹哈希（farmhash）生成初始哈希值h
- 通过位运算构造增量值delta = (h >> 33) | (h << 31)
- 采用双重哈希技术，通过循环叠加delta生成k个不同的位位置：h_i = h + i·delta (mod m)
3. 数据插入：
- 对输入key进行哈希计算得到初始h和delta
- 循环k次生成位位置，将位数组中对应位置设为1
- 采用位操作：byte_index = position/8，bit_mask = 1 << (position%8)
4. 存在性检测：
- 重复插入时的哈希计算过程
- 检查所有k个对应位是否均为1
- 任一位置为0则判定不存在，全部为1时判定可能存在
5. 数据持久化：
- 序列化时附加CRC32校验和
- 反序列化时验证校验和与数据完整性