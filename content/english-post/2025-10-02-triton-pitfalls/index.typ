...
Take the #link("https://triton-lang.org/main/python-api/generated/triton.language.load.html#triton.language.load")[tl.load documentation] as an example. It mentions that block pointers support "boundary checks" and "padding options," but:
...
- For raw pointers: Always set `mask` and `other`.
  - `mask=True`: Load from HBM.
  - `mask=False`: Use the value from `other`.
- For block pointers: Enable boundary checks on all dimensions and set `padding="zero"`.
