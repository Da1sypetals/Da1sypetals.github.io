...
#let f0 = $f_0$
#let fe = $F_e$
初步设想中，这个系统由两个模型组成：encoder和 f0 discriminator。
- encoder模型负责从mel中提取出#f0 无关的features，以“让#df0 无法从features里面提取出任何 #f0 信息”为优化目标；
- f0 discriminator (#df0) 从这个features里面尝试提取#f0，以“从features中提取出#f0”为优化目标。
通过这两个模型之间的对抗，希望让encoder features不含#f0 信息的同时，尽可能保留Mel频谱中的其他信息。

=== 如何定义“无法提取出任何 #f0 信息”
这需要先从神经网络如何描述#f0 说起。#f0 是一个标量，所以最自然的方法是把检测f0定义为一个回归问题；另一方面，现代#f0 检测模型（RMVPE，FCPE）则把f0检测定义为一个分类问题：把人类发声的可能f0按照一个公差进行等差数列分桶，每一个桶就是一个类别。类似的思路还可以在3D Mesh生成相关工作@polygen 里面看到。

对于我们的任务，有明确的理由应该选择后者，因为我们想要把“没有任何 #f0 信息”定义为一个数学上的目标，此时回归式的定义就不可用了：
- 一个单值输出无法表达“不确定性”或“无信息状态”。
- 例如，如果回归模型输出 200 Hz，模型无法区分这是“非常确定当前音高就是 200 Hz，还是“完全没有线索，只能瞎猜一个平均音高 200 Hz”。标量回归天然缺失了表示“熵（不确定度）”的自由度。
...
采用分类的方式后，可以把#f0 检测模块的输出定义为输出一个#f0 分桶上的离散分布，通过交叉熵损失优化#df0，通过和均匀分布$U$之间的KL散度优化encoder。
