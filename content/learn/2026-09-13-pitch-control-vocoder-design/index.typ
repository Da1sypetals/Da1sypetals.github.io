...
#let f0 = $f_0$
#let fe = $F_e$
我的目标是训练一个encoder从mel中提取出#f0 无关的features（encoder features #fe）, ；希望通过对抗训练的形式，训练一个f0 discriminator(#df0) 从这个features里面尝试提取#f0 ；然后再将“这个#df0 无法从我的 #fe 里面提取出任何 #f0 信息”这个目标，以一定权重加入到训练Encoder使用的那个loss里面。
...
按照这样，可以把#f0 检测模块的输出定义为输出一个#f0 分桶上的离散分布，通过交叉熵损失和KL散度的方式，分别进行#df0 和Encoder的优化。
...
最终的网络结构设计就是一个ConvNeXt架构@convnext 的非压缩式Autoencoder；#df0 部分则是直接照抄了FCPE@fcpe 的模型架构。
